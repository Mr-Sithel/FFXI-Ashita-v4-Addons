...
local TableDef = {
    
    {
        ...
        Draw = function(outpost, places) -- player,
            if imgui.Text(outpost.HomeNation) then
            AshitaCore:GetChatManager():QueueCommand(1, '/echo hello');
        end,
    },
    {
        Name = 'Jeuno',
...
local function DrawPlace(places)
    ...
    if imgui.BeginTable('xitools.lockstyle.summary', #TableDef, TableFlags) then
...
local lockstyle = {
    ...
    DrawConfig = function(options, gOptions)
        if imgui.BeginTabItem('lockstyle') then
